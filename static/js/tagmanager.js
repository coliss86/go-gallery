...
(function ($) {
  ...
  publicMethods = {
    pushTag: function (tag, ignoreEvents, externalTagId) {
      ...
      if (alreadyInList) {
        ...
        html += '<span id="t' + newTagId + '">' + escaped + '</span>';
  ...
  privateMethods = {
    ...
    selectTag: function (tagId) {
      var $self = this, tlis = $self.data("tlis"), tlid = $self.data("tlid"), tag;

      var id = parseInt(tagId.substring(tagId.indexOf("_") + 1));
      var idx = $.inArray(id, tlid);
      if (-1 !== idx) {
        tag = tlis[idx];

        if (!$("#" + tagId).hasClass("tm-tag-selected")) {
          $("#" + tagId).addClass("tm-tag-selected");
          $self.trigger('tm:selected', [tag, tagId]);
        } else {
